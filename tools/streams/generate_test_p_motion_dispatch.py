#!/usr/bin/env python3
"""Generate the controlled 128x96 H.262 P per-macroblock motion-dispatch regression.

Across the six P slice rows, the +32 aligned motion macroblock moves from column
0 through column 5.  The following macroblock is skipped to reset PMV, and all
remaining coded macroblocks use zero motion.  The script patches a deterministic
FFmpeg I/P/I skeleton and validates the decoded Y/Cb/Cr result pixel-for-pixel.
"""
...
def row_payload(shift_column: int) -> bytes:
    # quantiser_scale_code=2, extra_bit_slice=0
    bits = "000100"
    previous = -1
    for column in range(MB_WIDTH):
        ...
        if column == shift_column:
            # Table B.3 type 001; f_code=3 +32 horizontal uses motion_code +8,
            # residual 3; vertical motion_code is zero.
...
def patch_p_picture(data: bytes) -> bytes:
    ...
    region = [(index, offset, code) for index, (offset, code) in enumerate(codes)
              if p_picture < offset < next_picture and 1 <= code <= MB_HEIGHT]
    if tuple(code for _, _, code in region) != tuple(range(1, MB_HEIGHT + 1)):
        raise SystemExit("unexpected P slice layout")
    replacements = [(offset + 4, codes[index + 1][0]) for index, offset, _ in region]
...
def expected_p_from_i(frame: bytes) -> bytes:
    ...
    c_size = cw * ch
    out = bytearray(frame)
    for row, shift_column in enumerate(range(MB_HEIGHT)):
...
def verify(ffmpeg: str, ffprobe: str, output: Path) -> None:
    ...
    if slices != [(row + 1, ROW_PAYLOADS[row]) for row in range(MB_HEIGHT)]:
    ...
    if p_frame != expected_p_from_i(i_frame):
        raise SystemExit("verification failed: P frame differs from expected per-row motion plan")
    if p_frame == i_frame:
...
def main() -> None:
    ...
    output = Path(__file__).resolve().parent / "test_p_motion_dispatch.m2v"
    with tempfile.TemporaryDirectory(prefix="mister_h262_dispatch_") as temp_dir:
        temp = Path(temp_dir)
    ...
    for row, payload in enumerate(ROW_PAYLOADS, start=1):
        print(f"P slice {row:02x}: shift column {row - 1}, payload {payload.hex(' ')}")
    print("forward f_code: (3,3); shift-right execution map: 0x201008040201")

#!/usr/bin/env python3
"""Generate a controlled 96x64 / 6x4 / twenty-four-macroblock MPEG-2 P regression."""
...
HEIGHT=64
FPS=25
SLICE_CODES=(0x01,0x02,0x03,0x04)
SLICE_PAYLOAD=bytes.fromhex('12 79 e7 9e 79 c0')
...
def make_source_frame()->bytes:
    # Four rows by six columns of distinct constant luma macroblocks.
    vals=(
        (32,56,80,104,128,152),
        (48,72,96,120,144,168),
        (64,88,112,136,160,184),
        (80,104,128,152,176,208),
    )
...
def main()->None:
    ap=argparse.ArgumentParser(); ap.add_argument('-o','--output',type=Path,default=Path(__file__).resolve().parent/'test_p_twentyfour_mb_four_row.m2v'); args=ap.parse_args()
    ffmpeg=require_tool('ffmpeg'); ffprobe=require_tool('ffprobe'); args.output.parent.mkdir(parents=True,exist_ok=True)
    with tempfile.TemporaryDirectory(prefix='mister_h262_twentyfourmb_') as td:
        td=Path(td); raw=td/'twentyfour_mb.yuv'; skel=td/'twentyfour_mb_skeleton.m2v'
    ...
    print('P slices: 01/02/03/04 payload 12 79 e7 9e 79 c0')

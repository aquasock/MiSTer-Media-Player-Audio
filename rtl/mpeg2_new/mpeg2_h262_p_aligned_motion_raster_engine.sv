//============================================================================
// MiSTer Media Player - controlled aligned non-zero P raster engine
//
...
//
// Phase 1U-k controlled execution model for the 128x96 / 8x6 regression:
// destination macroblock column 0 predicts from reference macroblock column 1
// in each row. Column 1 is a standards-defined skipped P macroblock and all
// remaining macroblocks use zero-vector prediction, so they are copied from the
// colocated reference position. There are no residual coefficients in this
// controlled stream. This fixed pattern is diagnostic implementation scope, not
// an H.262 validity restriction.
//============================================================================

module mpeg2_h262_p_aligned_motion_raster_engine
(
    input  wire        clk,
    input  wire        reset,
    input  wire        request,
...
wire [8:0] reference_mb_col = (mb_col == 9'd0) ? 9'd1 : mb_col;
...
always @(posedge clk) begin
    if (reset) begin
        ...
        if (request && !started) begin
            ...
            row_index                <= 3'd0;
            timeout                  <= 24'hFFFFFF;
            if (!reference_valid || (destination_bank == reference_bank))
                error <= 1'b1;
            else
                request_active <= 1'b1;
        end

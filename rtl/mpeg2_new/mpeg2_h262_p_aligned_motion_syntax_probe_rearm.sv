//============================================================================
// MiSTer Media Player - re-armable controlled aligned non-zero H.262 P motion observer
//
...
//
// Phase 1U-n preserves the accepted controlled patterns and re-arms the proof
// state only when the already-consumed next picture header is itself P.  This
// retains sequence geometry while allowing consecutive P pictures.
//
    ...
    if (reset) begin
        ...
        if (picture_capture) begin
            ...
            if (picture_count) begin
                ...
                // The previous P proof completes on this picture's start code.
                // If this header is another P picture, clear only per-picture
                // proof state after the two header bytes have been captured.
                // Keeping aligned_candidate asserted provisionally suppresses
                // stale generic-P outputs until the new PCE validates controls.
                if (aligned_seen && (picture_next[5:3] == 3'd2)) begin
                    aligned_candidate       <= 1'b1;
                    aligned_seen            <= 1'b0;
                    aligned_shift_right_map <= 48'd0;
                    slice_capture           <= 1'b0;
                    slice_row_number        <= 4'd0;
